...
export default function DecryptedText({
  ...
  const wasExternalHoverRef = useRef(false);
  const triggerHoverDecryptRef = useRef(() => {});
  const resetToPlainTextRef = useRef(() => {});
  ...
  triggerHoverDecryptRef.current = triggerHoverDecrypt;
  resetToPlainTextRef.current = stopHoverAnimation;
  ...
  useEffect(() => {
    ...
    if (!externalHover) {
      if (wasExternalHoverRef.current) {
        resetToPlainTextRef.current();
      }
      wasExternalHoverRef.current = false;
      return;
    }

    wasExternalHoverRef.current = true;
  }, [externalHover]);

  useEffect(() => {
    if (externalHover === undefined || hoverTrigger === 0) return;
    if (!externalHover) return;

    triggerHoverDecryptRef.current();
  }, [hoverTrigger, externalHover]);
