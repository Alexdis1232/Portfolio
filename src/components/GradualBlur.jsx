...
function GradualBlur(props) {
  ...
  const [pageScrollVisible, setPageScrollVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    const updateVisibility = () => {
      ...
      const atPageEnd = scrollTop >= maxScroll - 24;

      setPageScrollVisible(
        hasScrolledRef.current || atPageEnd || maxScroll <= 20
      );
    };
  ...
  const isBottomPageBlur =
    config.target === 'page' && config.position === 'bottom';

  const showBlur = isBottomPageBlur
    ? config.animated === 'scroll'
      ? pageScrollVisible
      : true
    : config.target === 'page' && config.animated === 'scroll'
      ? pageScrollVisible
      : config.animated !== 'scroll' || isVisible;
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle = {
      ...
      opacity: showBlur ? config.opacity : 0,
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, showBlur]);

  const { hoverIntensity, animated, onAnimationComplete, duration } = config;

  useEffect(() => {
    if (showBlur && animated === 'scroll' && onAnimationComplete) {
      const ms = parseFloat(duration) * 1000;
      const t = setTimeout(() => onAnimationComplete(), ms);
      return () => clearTimeout(t);
    }
  }, [showBlur, animated, onAnimationComplete, duration]);
